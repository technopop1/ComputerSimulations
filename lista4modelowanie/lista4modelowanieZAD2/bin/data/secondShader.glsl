...
vec4 color(float t)
{
	...
	col.r = coltab[4]+coltab[1]*cos(4*3.1416*(coltab[6]*t+coltab[9]));
	col.g = coltab[5]+coltab[2]*cos(2.5*3.1416*(coltab[7]*t+coltab[10]));
	col.b = coltab[6]+coltab[3]*cos(0.6*3.1416*(coltab[8]*t+coltab[11]));
	col.a = 1;
	return col;
}
...
void main(){ 
	...
	idx8 = i+(jp)*W; // i, j+1
	// laplacians
	float laplA = -1.0 * A1[idx0] + .2 * (A1[idx6] + A1[idx2] + A1[idx4] + A1[idx8]) + 0.05 * (A1[idx1] * dt  + A1[idx3] + A1[idx5] + A1[idx7]) ;
	float laplB = -1.0 * B1[idx0] * dt + .2 * (B1[idx6] + B1[idx2]+ B1[idx4] + B1[idx8]) + 0.05 * (B1[idx1] + B1[idx3] + B1[idx5] * dt  + B1[idx7]);
	// Gray Scott model
